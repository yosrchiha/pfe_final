// frontend/app/merge-requests/page.tsx
"use client";
...
const fetchProjets = async () => {
  ...
  try {
    ...
    if (res.data.length > 0) {
      ...
      fetchMergeRequests(res.data[0].id);
    }
  } catch (e: any) {
  ...
  // Charger les MR d'un projet
  const fetchMergeRequests = async (projetId: number) => {
    setLoadingMrs(true);
    try {
      const res = await axios.get(`${API}/merge-requests/depot/${projetId}`, { headers: getHeaders() });
      setMrs(res.data);
      
      const data = res.data;
      setStats({
        total: data.length,
        opened: data.filter((m: MergeRequest) => m.statut === "opened").length,
        merged: data.filter((m: MergeRequest) => m.statut === "merged").length,
        closed: data.filter((m: MergeRequest) => m.statut === "closed").length,
        tests: data.filter((m: MergeRequest) => m.type_mr === "tests").length,
        auto_merge: data.filter((m: MergeRequest) => m.type_mr === "auto_merge").length,
        diff: data.filter((m: MergeRequest) => m.type_mr === "diff").length,
      });
    } catch (e) {
  ...
  const selectionnerProjet = (projet: Projet) => {
    ...
    setSelectedMr(null);
    fetchMergeRequests(projet.id);
  };

  // Synchroniser le statut d'une MR
  const syncStatus = async (id: number) => {
    try {
      const res = await axios.put(`${API}/merge-requests/${id}/sync`, {}, { headers: getHeaders() });
      setMrs(prev => prev.map(mr => 
        mr.id === id ? { ...mr, statut: res.data.statut } : mr
      ));
      fetchMergeRequests(projetSelectionne!.id);
    } catch (e) {
  ...
  const typeConfig = (t: string) => {
    if (t === "tests") return { icon: "🧪", label: "Tests unitaires", color: "#6366f1", bg: "#eef2ff" };
    if (t === "auto_merge") return { icon: "⚡", label: "Auto-merge IA", color: "#f59e0b", bg: "#fffbeb" };
    return { icon: "📊", label: "Analyse diff", color: "#10b981", bg: "#ecfdf5" };
  };
        ...
        <div className="main-content">
          <div className="topbar">
            <div className="title-section">
              ...
              <p>
                {projetSelectionne 
                  ? `Merge Requests pour ${projetSelectionne.nom}`
          ...
          {projetSelectionne && (
            <>
              ...
              <div className="stats-grid">
                ...
                <div className="stat-card"><div className="stat-value" style={{ color: "#10b981" }}>{stats.merged}</div><div className="stat-label">Fusionnées</div></div>
                ...
                <div className="stat-card"><div className="stat-value">{stats.auto_merge}</div><div className="stat-label">⚡ Auto</div></div>
                <div className="stat-card"><div className="stat-value">{stats.diff}</div><div className="stat-label">📊 Diff</div></div>
              </div>
              ...
              <div className="filters">
                <div className="search-wrapper">
                  ...
                  <input className="search-input" placeholder="Rechercher par titre, branche..." value={search} onChange={e => setSearch(e.target.value)} />
                </div>
                ...
                <button className="sync-btn" onClick={() => fetchMergeRequests(projetSelectionne.id)}>↻ Rafraîchir</button>
              </div>
                        ...
                        return (
                          <tr key={mr.id} className={`table-row ${selectedMr?.id === mr.id ? "selected" : ""}`} onClick={() => setSelectedMr(mr)}>
                            ...
                            <td onClick={e => e.stopPropagation()}>
                              <button className="sync-btn" onClick={() => syncStatus(mr.id)} title="Synchroniser avec GitLab">
                                ↻ Sync
                              </button>
                            </td>
                          </tr>
            ...
            <>
              ...
              <div className="panel-content">
                ...
                <div className="info-group">
                  ...
                  <div className="info-value">ID #{selectedMr.analyse_id}</div>
                </div>
                {selectedMr.test_id && (
                  <div className="info-group">
